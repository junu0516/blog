...
- __시스템 호출__ 로 해석하며, 운영체제의 __`커널(kernel)`__ 이 제공하는 서비스에 대해, 응용 프로그램의 여러 요청에 따른 커널 접근을 위한 인터페이스를 말함
  - 쉽게 말해 사용자 모드에서 커널 모드로 전환해서 커널에 접근하는 것
...
1) __프로세스 컨트롤(Process Control)__
   ...
   - 프로세스를 실행하거나 종료하는 일을 수행하며 __`fork()`__ 가 실행되면 프로세스(자식 프로세스)가 생성되며, __`exec()`__ 가 실행되면 프로세스를 실행하게 됨.(보통 운영체제가 시작되면 PID=1인 부모 프로세스가 실행됨)
...
- **File modification** : md, properties 등의 확장자를 지닌 파일을 메모장과 같은 에디터 프로그램을 통해 수정하는 것을 생각하면 됨
...
- 운영체제의 커널은 일반적으로 시스템 호출을 통해 파일 시스템, CPU 스케쥴링, 메모리 관리 등의 기능을 수행함
- 운영체제 구조에 대한 접근은 커널이 모든 서비스를 포함하는 구조에서 부터 전체가 커널 외부의 독립적인 여러 모듈로 나뉘는 구조로 접근할 수 있음
- 코드로 치면 모든 로직이 __`main()`__ 안에 있는 것이 아니라, 모듈화를 한 후 여러 함수를 __`main()`__ 에서 필요에 따라 조합하여 호출하는 방식인 것
- 여러 부분이 커널에 합쳐지는 방식 또한 다양함
...
- 모놀리식 구조는 단일형 구조를 의미하며, 제일 간단한 접근 방식으로 모듈화 없이 운영체제의 기능이 단일한 메모리 공간 위에서 동작하도록 하는 방식임
  - 쉽게 말해 운영체제의 모든 서비스가 커널 내에 포함된 구조로 이해하면 됨
  - 오리지널 유닉스 구조가 이런 방식이며, 크게 커널과 시스템 프로그램의 두 부분으로 나뉜 형태였음
  - 이러한 구조였던 것이 유닉스가 진화하면서 커널이 여러 디바이스 드라이버와 인터페이스로 나뉘어 추가 및 확장되온 것
- 구조상으로는 단일한 것이 제일 간단하며 커널 내의 요소들 간 효율적인 상호작용이 가능하다는 장점이 있음
- 하지만 동시에 기능을 확장시키는 것이 힘들뿐더러 한 요소에 있는 오류가 전체 시스템의 오류를 야기할 위험이 크다는 단점이 있음
...
- 이는 운영체제에서 필수적이지 않는 부분을 커널에서 모두 제거해서 별도의 유저 레벨의 메모리 공간에서 커널과 함께 동작하도록 한 방식임
  - 쉽게 말해 운영체제의 여러 요소를 커널 외부로 분리시킨 구조인 것
